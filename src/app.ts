...
//#region my-awesome-cli-tool component
...
export class MyAwesomeCliToolApp implements OnInit {
  itemsLoaded = signal(false);

  navItems =
    MyAwesomeCliToolClientRoutes.length <= 1
      ? []
      : MyAwesomeCliToolClientRoutes.filter(r => r.path !== undefined).map(
          r => ({
            path: r.path === '' ? '/' : `/${r.path}`,
...
//#region  my-awesome-cli-tool api service
...
//#region  my-awesome-cli-tool routes
//#region @browser
export const MyAwesomeCliToolServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const MyAwesomeCliToolClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (MyAwesomeCliToolClientRoutes.length === 1) {
        return '';
      }
      return MyAwesomeCliToolClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  my-awesome-cli-tool app configs
//#region @browser
export const MyAwesomeCliToolAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
      provide: TAON_CONTEXT,
      useFactory: () => MyAwesomeCliToolContext,
    },
    providePrimeNG({
      theme: {
        preset: Aura,
    ...
    }),
    {
      ...
      multi: true,
      useFactory: () => MyAwesomeCliToolStartFunction,
    },
    provideBrowserGlobalErrorListeners(),
    // remove withHashLocation() to use SSR
    provideRouter(
      MyAwesomeCliToolClientRoutes,
      withHashLocation(),
      withComponentInputBinding(),
    ),
...
export const MyAwesomeCliToolServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(MyAwesomeCliToolServerRoutes))],
};

export const MyAwesomeCliToolConfig = mergeApplicationConfig(
  MyAwesomeCliToolAppConfig,
  MyAwesomeCliToolServerConfig,
);
//#endregion
//#endregion

//#region  my-awesome-cli-tool entity
...
//#region  my-awesome-cli-tool controller
...
//#region  my-awesome-cli-tool migration
...
//#region  my-awesome-cli-tool context
var MyAwesomeCliToolContext = Taon.createContext(() => ({
  ...HOST_CONFIG['MyAwesomeCliToolContext'],
...
//#region  my-awesome-cli-tool start function
export const MyAwesomeCliToolStartFunction = async (
  startParams?: Taon.StartParams,
): Promise<void> => {
  await MyAwesomeCliToolContext.initialize();
...
export default MyAwesomeCliToolStartFunction;
//#endregion
